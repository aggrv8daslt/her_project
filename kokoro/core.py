...
def generate(model, text, speaker_embedding, lang='a', speed=1.0, ps=None, play_audio=True, return_wav=False):
    ...
    if not tokens:
        print("[generate] No tokens generated!")
        return None, None
    ...
    print(f"[DEBUG] generate(): tokens.shape = {tokens.shape if hasattr(tokens, 'shape') else 'N/A'}")
    print(f"[DEBUG] generate(): ref_s shape = {ref_s.shape if ref_s is not None else 'None'}")
